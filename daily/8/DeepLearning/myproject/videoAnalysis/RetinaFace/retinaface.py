...
class RetinaFaceDetector():
    def __init__(self,modelpath,thresh=0.8,scale=[1024,1980],device='cpu'):
        gpuid = -1 if device=='cpu' else 0
        self.detector = RetinaFace(modelpath, 0, gpuid, 'net3')
        self.scales=scale
        self.target_size=self.scales[0]
        self.max_size = self.scales[1]
        self.thresh=thresh
    def detect_faces(self,pImg,*args,**kwargs):
        '''
        ...
        '''
        im_shape = pImg.size
        ##############determine the scale of argument########################
...
if __name__ == '__main__':
    m=RetinaFaceDetector('model/R50',device='cuda')

    import PIL.Image as Image
    p=Image.open('t1.jpg')
    for i in range(10):
        faces,landmarks=m.detect_faces(p)
        print(faces.shape)
