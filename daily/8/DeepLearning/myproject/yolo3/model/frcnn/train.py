...
class DetectionModel(ModelDesc):
    ...
    '''
        AccumGradOptimizer是一个积累gradient然后更新的方法,
        例如:想要BATCH_size=256,但是GPU显存只允许128,
        可以计算2次gradient然后进行一次梯度更新.
        
        理论上,lr的设置应该随着batch_size的增大而减小
        lr(128)=lr(256)*2..
        
        这里默认基本学习率是3e-3,使用8的GPU,每个GPU学习率是3e-3/8
        
        这里使用8个GPU,表示计算8次GRadent更新一次梯度,
        所有#gpu<8使用,要把AccumGradOptimizer的参数设在为8/#gpu
    ...
    def get_inference_tensor_names(self):
        ...
        return ['image'], out
    '''
    inputs=[image,ahchor_*,gt_boxes,gt_labels,gt_masks]
    
    提取特征features
    rpn:prososal,prososal_loss=rpn(image,features,ahchor_*)
    roi:roi_loss=roi(image,features,prososal,[gt_boxes,gt_labels,gt_masks])
    
    total_loss=roi_loss+proposal_loss+regular_loss
    '''
...
class ResNetC4Model(DetectionModel):
    ...
    def inputs(self):
        ...
        image:(?,?,3)float
        anchor_labels:(?,?,9)int
        anchor_boxes:(?,?,9,4)float
        gt_label(#props)int
        gt_label(#props,4)float
        :return: 
        '''
        ret = [
    ...
    def roi_heads(self, image, features, proposals, targets):
        ...
        :param image: (1,H,W,3)
        :param features: (1,H/16,W/16,1024)
        :param proposals: (n,4),
        :param targets: (m,4)
        :return: 
        '''
        ...
        #feature map坐标与原图坐标转化
        boxes_on_featuremap = proposals.boxes * (1.0 / cfg.RPN.ANCHOR_STRIDE)
        #给出了特征值,和感兴趣的区域,那么就用一种特殊的办法把感兴趣区域封装成14X14的区域把
...
def do_visualize(model, model_path, nr_visualize=100, output_dir='output'):
    """
    Visualize some intermediate results (proposals_boxes, raw predictions) inside the pipeline.
    """
    ...
    with tqdm.tqdm(total=nr_visualize) as pbar:
        for idx, dp in itertools.islice(enumerate(df), nr_visualize):
            ...
            # draw best proposals_boxes for each groundtruth, to show recall
            proposal_viz, good_proposals_ind = draw_proposal_recall(img, rpn_boxes, rpn_scores, gt_boxes)
            # draw the scores for the above proposals_boxes
...
if __name__ == '__main__':
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        #warmup_schedule:[(0,init_lr),(warnup_step,lr)]
        #lr_schedule    :[(1,lr),(e1,lr/10),(e2,lr/100),(e3,lr/1000)...]
