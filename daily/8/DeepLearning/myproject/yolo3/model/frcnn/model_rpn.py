...
def generate_rpn_proposals(boxes, scores, img_shape,
    ...
    """
    给出proposal boxes,给出所有decodedboxes,和分数,执行流程如下 
    1.选出分数最高pre_nms_topk个boxes
    2.对上面的boxes做nms
    3.对nms输出boxes选出最高分数的post_nms_topk
    Sample RPN proposals_boxes by the following steps:
    ...
    # nx1x2 each,第二部,过滤掉分数不合法的,或者说过滤掉区域过小的
...
def rpn_losses(anchor_labels, anchor_boxes, label_logits, box_logits):
    """
    anchor_labels:标注的0,1,-1
    anchor_boxes:encoded的编码好的
    返回boxes_loss,label_loss
    Args:
...
class RPNAnchors(namedtuple('_RPNAnchors', ['boxes', 'gt_labels', 'gt_boxes'])):
    ...
    #网络的相对坐标还原成绝对坐标,(h,w,a,4)
    def decode_logits(self, logits):
        return decode_bbox_target(logits, self.boxes,cfg.PREPROC.MAX_SIZE)

    @under_name_scope()
    def narrow_to(self, featuremap):
        """
        Slice anchors to the spatial size of this featuremap.
        根据feature的长宽,截取boxes,gt_labels,gt_boxes,返回新的 RPNAnchors
        """
        ...
        gt_labels = tf.slice(self.gt_labels, [0, 0, 0], slice3d) if self.gt_labels else None
        gt_boxes = tf.slice(self.gt_boxes, [0, 0, 0, 0], slice4d) if self.gt_boxes else None
