...
class BaseModel(object):
  ...
  #self.embeding_encoder,self.embeding_decoder
  ...
  def _build_encoder(self, hparams):
    ...
    '''
    调用model_helper返回一个完整的RNN单元tf.contrib.rnn.MultiRnnCell()
    '''
...
class Model(BaseModel):
  ...
  """
  '''
  sequence:(N,Tmax,embed_size),序列数据
  sequence_length:(N,)数据真实长度
  
  通过一个LSTM sequence,并且更新
  self.encoder_state_list=[outputs]
  返回:
    outputs:(N,Tmax,ln_dims)最后一层维度,(N,Tmax,ln_dims*2) for encoder_type=='bi'
    states:final_states,((c0,h0),(c1,h1)....(cn,hn)),(ci,hi)维度由layer[i].dim决定!
  ...
  def _build_encoder_from_sequence(self, hparams, sequence, sequence_length):
    ...
    Args:
      ...
      sequence: tensor with input sequence data.shape:[batch,Tx]
      sequence_length: tensor with length of the input sequence.shape:[batch,)
    ...
    with tf.variable_scope("encoder") as scope:
      dtype = scope.dtype
     #输入是sequence,shape[N,T],经过embed,变成了encoder_emb_inp,shape[N,T,embed_size]
      ...
      if hparams.encoder_type == "uni":
        ...
        '''
        unroll 单个cell,
        输入(encoder_emb_inp)是shape[BATCH,Tmax,embed_size],
        sequence_length:(BATCH,)表示每行数据的实际长度,这样才能保证backpropgate的正确性
        ,例如Tmax=10,sequence_len=7,1-7输出产生的loss会反向传播,8-10loss不会
        time_major:表示T是否是第一维度
        swap_memory:把计算forword的tensor(这些tensor参与backpropgate)从GPU交换到CPU,
        保证如何GPU显存不够用,使用物理内存,而不会OOM
        返回:
            encoder_outputs:[batch_size,Tmax,cell.output_size],
                cell.output_size是最高层LSTM的单元
            encoder_state:final state,[batch_size,Tmax,cell.state_size],
            例如有3层网络,ndims=[32,64,128]
            
            encoder_outputs:shape [Batch,Tmax,128]
            encoder_state:((c0,h0),(c1,h1),(c2,h2))
                c0,h0:[Batch,32]
                c1,h1:[Batch,64]
                c2,h2:[Batch,128]
...
'''
  ...
  '''
  inputs:(Batch,Tmax,embed_size)
  sequence_length:(Batch,)
  
  输出:
    outputs:(Batch,2*ln_dims)
    states:([(c0,h0),(c1,h1),(c2,h2)],[(c'0,h'0),(c'1,h'1),(c'2,h'2)])
  ...
  def _build_bidirectional_rnn(self, inputs, sequence_length,
    ...
    '''
    bi_outputs:(forward_output,backward_output),shape (N,ln_dims)
    bi_state:(forward_state,backward_state)
        forward_state:(l1_state,....ln_state,)
            li_state:(ci,hi)
    '''
