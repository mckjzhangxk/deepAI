...
检查单词文件vocab_file,是否存在,不存在抛异常
  check_special_token:如果TRUE,
  检测vocab_file头3个是不是unk,sos,eos
    如果不是:输出一个新文件out_dir/vocab_file.basename
    这个新文件头3个单词是unk,sos,eos
...
'''
  根据源单词文件,目标单词文件,
  生成基于0的int索引,返回HASHTABLE的
  src_vacab_table,tgt_vocab_table,
  share_vocab==true表示源和目标是一个文件返回的src_vacab_table==tgt_vacab_table
'''
...
返回:
  embed_dict:key:word,value:array[embedsize]
    例如hello:[0.1,-0.2,......]
  embedsize:
'''
