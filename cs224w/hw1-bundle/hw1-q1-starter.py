...
def genErdosRenyi(N=5242, E=14484):
    ...
    nodes=list(range(N))
    for n in nodes:Graph.AddNode(n)
    for _ in range(E):
        while True:
...
def genCircle(N=5242):
    ...
    nodes=list(range(N))
    for n in nodes:Graph.AddNode(n)
    for n in range(N):Graph.AddEdge(n,(n+2)%N)
...
def connectRandomNodes(Graph, M=4000):
    ...
    ############################################################################
    # TODO: Your code here!
    nodes=list(range(Graph.GetNodes()))
    for _ in range(M):
        while True:
            e=np.random.choice(nodes,size=(2,),replace=False)
            if not Graph.IsEdge(int(e[0]),int(e[1])):
...
def getDataPointsToPlot(Graph):
    ...
    # TODO: Your code here!
    X, Y = [], []
    degree_dist=snap.TIntPrV()
    snap.GetDegCnt(Graph,degree_dist)

    for element in degree_dist:
        X.append(element.GetVal1())
        Y.append(element.GetVal2())
    sumcnt=sum(Y)
...
def calcClusteringCoefficientSingleNode(Node, Graph):
    ...
    E=0
    for i in range(len(neibors)-1):
        for j in range(i+1,len(neibors)):
            E+=Graph.IsEdge(neibors[i],neibors[j])
    C=2*E/(K*(K-1))
    ############################################################################
    return C
