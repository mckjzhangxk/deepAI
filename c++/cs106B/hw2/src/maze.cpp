...
Maze::Maze(int numRows, int numCols, bool hasWalls) : cells(numRows, numCols) 
{
	for (int r = 0; r < numRows; r++)
        for (int c = 0; c < numCols; c++){
            cellT cell=cells.get(r,c);
            for (int d = 0; d < NumDirs; d++){
                cell.walls[d]=hasWalls;
            }
            cells.set(r,c,cell);
        }
...
bool Maze::isWall(pointT p1, pointT p2)
{
	if (!pointInBounds(p1) || !pointInBounds(p2))
        throw "Point is not in bounds for maze";
    return cells.get(p1.row, p1.col).walls[neighborDir(p1, p2)];
}
...
void Maze::setWall(pointT p1, pointT p2, bool state)
{
    ...
    cellT c1=cells.get(p1.row,p1.col);
    cellT c2=cells.get(p2.row,p2.col);


    c1.walls[neighborDir(p1, p2)] = state;
    c2.walls[neighborDir(p2, p1)] = state;
...
Maze::dirT Maze::neighborDir(pointT p1, pointT p2)
{
	if ((abs(p1.row-p2.row) + abs(p1.col-p2.col)) != 1)
        throw "Points are not neighbors";
	if (p1.row != p2.row)
...
void Maze::drawWallsForCell(pointT p)
{
    ...
    SetPenColor(cells.get(p.row, p.col).walls[South] ? "Black" : "White");
	DrawLine(cellSize, 0);
    SetPenColor(cells.get(p.row, p.col).walls[East] ? "Black" : "White");
	DrawLine(0, cellSize);
    SetPenColor(cells.get(p.row, p.col).walls[North] ? "Black" : "White");
	DrawLine(-cellSize, 0);
    SetPenColor(cells.get(p.row, p.col).walls[West] ? "Black" : "White");
	DrawLine(0, -cellSize);
}
